...
This is a monorepo for the **WSO2 O2C QR Portal** — a conference QR code management system. It contains two apps under `apps/qr-portal/`:
- **backend**: Ballerina REST microservice (port 9090)
- **webapp**: React/TypeScript frontend (port 3000)

## Commands

### Frontend (`apps/qr-portal/webapp/`)
```bash
...
### Backend (`apps/qr-portal/backend/`)
```bash
...
| File | Purpose |
|------|---------|
| `apps/qr-portal/backend/Config.toml` | Local runtime config (not committed) |
| `apps/qr-portal/webapp/public/config.js` | Frontend runtime config (not committed) |
| `apps/qr-portal/backend/README.md` | Full API reference with request/response examples |
| `apps/qr-portal/webapp/README.md` | Frontend setup guide |
